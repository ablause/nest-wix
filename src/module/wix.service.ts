...
import { WIX_AUTH_BASE_URL, WIX_MODULE_OPTIONS, WIX_APIS_BASE_URL } from './wix.constant'
...
export class WixService {
  ...
  public async getTokensFromWix (authCode: string): Promise<TokensFromWixResult> {
    ...
    const { href } = new URL('/oauth/access', WIX_AUTH_BASE_URL)
  ...
  public async getAppInstance (refreshToken): Promise<WixAppInstance> {
    try {
      ...
      const { href } = new URL('/apps/v1/instance', WIX_APIS_BASE_URL)

      const { data } = await this.httpService.get<WixAppInstance>(href, {
